...
class YumyumApplicationTests {
	...
	public void testOfInsert() {
		BoardDTO params = new BoardDTO();
			...
			params.setUserNum(3);
			params.setTitle("insert Test");;
			params.setWriterId("3 id");
			params.setContent("1번 게시글 테스트입니다.");
	...
	public void testOfSelectDetail() {
		BoardDTO board = boardMapper.selectBoardDetail(3);
		try {
	...
	public void testOfUpdate() {
		...
		params.setWriterId("홍길동");
		params.setUserNum(3);

		int result = boardMapper.updateBoard(params);
		if (result == 1) {
			BoardDTO board = boardMapper.selectBoardDetail((int) 3);
			try {
				//String boardJson = new ObjectMapper().writeValueAsString(board);
                String boardJson = new ObjectMapper().registerModule(new JavaTimeModule()).writeValueAsString(board);
	...
	public void testMultipleInsert() {
		for (int i = 2; i <= 50; i++) {
			...
			params.setWriterId(i + "번 게시글 작성자");
